...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    dispatch_block_t dismissCompletionBlock = ^{
        ...
        if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
            ...
            CLLocationDegrees longitude = [self.options[@"longitude"] doubleValue];
            CLLocationDegrees latitude = [self.options[@"latitude"] doubleValue];
            double altitude = [[self.options objectForKey:@"altitude"] doubleValue];
            
            NSMutableArray *metadata = [NSMutableArray array];
            AVMutableMetadataItem *metaItem = [AVMutableMetadataItem metadataItem];
            metaItem.key = AVMetadataQuickTimeMetadataKeyLocationISO6709;
            metaItem.identifier = AVMetadataIdentifierQuickTimeMetadataLocationISO6709;
            metaItem.value = [NSString stringWithFormat:@"%+09.5f%+010.5f%+.0fCRSWGS_84", latitude, longitude, altitude];
            [metadata addObject:metaItem];
            
            NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)objectAtIndex:0];
            NSString *videoFileName = [@"/" stringByAppendingString:[[NSUUID UUID] UUIDString]];
            videoFileName = [videoFileName stringByAppendingString:@".mov"];
            NSString *destinationPath = [documentsDirectory stringByAppendingString:videoFileName];
            ...
            [exportSession exportAsynchronouslyWithCompletionHandler:^{
                switch ([exportSession status])
                {
                    case AVAssetExportSessionStatusCompleted:
                        [self.response setObject:destinationPath forKey:@"uri"];
                        self.callback(@[self.response]);
                        break;
